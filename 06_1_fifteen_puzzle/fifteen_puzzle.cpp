...
string construct_candidates(Game &game,
    ...
    struct Candidate_Info {
        ...
        Candidate_Info(char c, int d) : candidate(c), distance(d) {}
    };
    ...
    for (char c : candidates) {
        ...
        if (visited_game_states.find(game.key()) == visited_game_states.end()) {
            ...
            int movesTotal = game.get_moves_so_far().size();

            if (movesTotal + distance <= 200) {
                visited_game_states.insert(game.key());
                new_candidates.push_back(Candidate_Info(c, distance));
            }
        }
