...
inline void clean_coconut(string &s) {
    char *pch = const_cast<char *>(strstr(s.data(), "Coconut"));
    while (pch != nullptr) {
        char *digits = pch + strlen("Coconut");
        size_t digitCount = 0;
        while (isdigit(*digits) && digitCount < 6) {
            ++digits;
            ++digitCount;
        }
        ...
        pch = const_cast<char *>(strstr(pch + strlen("Coconut") + digitCount, "Coconut"));
    }
}
...
inline void clean_spaces(string &s) {
    char *pch = const_cast<char *>(strstr(s.data(), "  "));
    while (pch != nullptr) {
        size_t spaceCount = 0;
        while (pch[spaceCount] == ' ' && spaceCount < 2) {
            ++spaceCount;
        }
        if (spaceCount == 2) {
            pch[1] = ' ';
            memmove(pch + 2, pch + 3, strlen(pch + 2));
        }
        pch = const_cast<char *>(strstr(pch + 1, "  "));
    }
}
