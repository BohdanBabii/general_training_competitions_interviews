...
inline uint32_t reverse_with_lookup(uint32_t num) {
    ...
    uint8_t byte1 = static_cast<uint8_t>((num >> 24) & 0xFF);
    uint8_t byte2 = static_cast<uint8_t>((num >> 16) & 0xFF);
    uint8_t byte3 = static_cast<uint8_t>((num >> 8) & 0xFF);
    uint8_t byte4 = static_cast<uint8_t>(num & 0xFF);

    result = (static_cast<uint32_t>(lookup[byte4]) << 24) |
             (static_cast<uint32_t>(lookup[byte3]) << 16) |
             (static_cast<uint32_t>(lookup[byte2]) << 8) |
             (static_cast<uint32_t>(lookup[byte1]) << 0);
